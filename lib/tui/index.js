...
const { cursorPosition, eraseDisplay, eraseLine } = require('bare-ansi-escapes')
...
class Text extends Element {
  constructor(
    ...
    y,
    text,
    { color = '', width = 100, paddingX = 1, paddingY = 0 } = {}
  ) {
...
class SelectableList extends Element {
  constructor(
    ...
    width,
    height,
    { items = [], onSelect = null, onCopy = null } = {}
  ) {
    super(x, y, width, height)
    this.items = items
  ...
  _createTextElements() {
    ...
    this.items.forEach((item, index) => {
      ...
      const textElement = new Text(0, index, text, {
        color: 'yellow',
        paddingX: 2
      })
  ...
  render(parentWidth, parentHeight) {
    ...
    this.textElements.forEach((textElement, index) => {
      const { pixels: textPixels, width: textWidth } = textElement.render(
        width,
        height
      )
...
class Tui {
  ...
  buffer() {
    ...
    for (const child of this.children) {
      const { pixels: childPixels, width: childWidth } = child.render(
        this.width,
        this.height
      )
      ...
      for (let i = 0; i < childPixels.length; i++) {
        ...
        const childX =
          typeof child.x === 'function'
            ? child.x(this.width, this.height)
            : child.x
        const childY =
          typeof child.y === 'function'
            ? child.y(this.width, this.height)
            : child.y

        const targetX =
          childX < 0
            ? this.width + childX + childXOffset
            : childX + childXOffset
        const targetY =
          childY < 0
            ? this.height + childY + childYOffset
            : childY + childYOffset
