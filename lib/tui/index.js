...
class SelectableList extends Element {
  ...
  _createTextElements () {
    ...
    this.items.forEach((item, index) => {
      const text = `• ${item.name || 'Unnamed Item'} - ${item.value || ''}`
  ...
  buffer () {
    ...
    for (const child of this.children) {
      ...
      for (let i = 0; i < childPixels.length; i++) {
        const childX = i % childWidth
        const childY = Math.floor(i / childWidth)

        const targetX = child.x < 0 ? this.width + child.x + childX : child.x + childX
        const targetY = child.y < 0 ? this.height + child.y + childY : child.y + childY
...
module.exports = { Tui, Box, Text, SelectableList }
