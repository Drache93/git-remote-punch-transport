...
class PunchLocalDB extends ReadyResource {
  ...
  async _open() {
    this._identity = await getIdentity()
    if (!this._identity) {
      throw new Error('No identity found, please run `config -u <username>`')
    }

    const keyPair = {
      publicKey: this._identity.devicePublicKey,
      secretKey: this._identity.deviceSecretKey
    }

    this._swarm = new Hyperswarm({ keyPair })
