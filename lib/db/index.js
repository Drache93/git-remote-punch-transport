...
const { PunchRemoteDB } = require('./remote2')
...
class PunchLocalDB extends ReadyResource {
  ...
  async *getRemotes(query, options = {}) {
    const knownRepos = await this._db.find('@punch/repos', query, options)
  ...
  // @todo remove
  async openRemotes() {
    const knownRepos = await this._db.find('@punch/repos')
  ...
  async createRemote(name) {
    ...
    await this._db.insert('@punch/repos', {
      name,
      key: remote.key
    })
  ...
  async openRemote(opts = {}) {
    const repo = await this._db.get('@punch/repos', opts)
    if (repo) {
      const remote = new PunchRemoteDB({
        ...repo,
        swarm: this._swarm,
    ...
    return this.joinRemote(opts)
  }

  async joinRemote(opts = {}) {
    const remote = new PunchRemoteDB({
      ...opts,
      swarm: this._swarm,
      store: this._store,
    ...
    this._remotes.set(opts.name, remote)
  ...
  async getRepo(name) {
    const repo = await this._db.get('@punch/repos', { name })
    if (!repo) {
      return null
    }
