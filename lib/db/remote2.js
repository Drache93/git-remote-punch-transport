...
class PunchRemoteDB extends ReadyResource {
  ...
  get core () { return this._bee.core }
  get key () { return this._bee.core.key }
  get discoveryKey () { return this._bee.core.discoveryKey }
  get availablePeers () { return this._bee.core.peers.length }
  ...
  async getRefObjects (commitOid, onLoad) {
    ...
    for (const oid of branch.objects) {
      ...
      if (obj.size !== obj.data.length) {
        throw new Error(`Invalid object size for ${oid}`)
      }
...
class PunchDrive extends ReadyResource {
  ...
  _findRange (folder) {
    ...
    if (!folder || folder === '/') {
      return this._db.find('@punch/files', { branch })
    }

    return this._db.find('@punch/files', {
