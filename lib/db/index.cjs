const { homedir } = require('os')
const { join } = require('path')
const Corestore = require('corestore')
const BlindPeering = require('blind-peering')
const Hyperswarm = require('hyperswarm')
const Wakeup = require('protomux-wakeup')
const ReadyResource = require('ready-resource')
const HyperDB = require('hyperdb')
const def = require('./local/schema/hyperdb/index')
const { PunchRemoteDB } = require('./remote2.cjs')

const blindPeers = ['y9frctpdhd5cp3hxrkn51ih89ghcpfeuzkixkibmsa65b66oea3y']

class PunchLocalDB extends ReadyResource {
  _swarm = null
  _store = null
  _wakeup = null
  _identity = null
  _db = null
  _remotes = new Map()

  constructor(args = {}) {
    super()

    this._store = args.store || new Corestore(join(homedir(), '.punch'))
  }

  // @todo remove
  get remotes() {
    return this._remotes
  }

  getRemote(repo) {
    return this._remotes.get(repo)
  }

  async *getRemotes(query, options = {}) {
    const knownRepos = await this._db.find('@punch-local/repos', query, options)

    for await (const repo of knownRepos) {
      const remote = new PunchRemoteDB({
        ...repo,
        swarm: this._swarm,
        store: this._store,
        blind: this._blind
      })

      await remote.ready()

      yield remote
    }
  }

  // @todo remove
  async openRemotes() {
    const knownRepos = await this._db.find('@punch-local/repos')

    for await (const repo of knownRepos) {
      const remote = new PunchRemoteDB({
        ...repo,
        swarm: this._swarm,
        store: this._store,
        blind: this._blind
      })

      await remote.ready()

      this._remotes.set(repo.name, remote)
    }

    return this._remotes
  }

  async createRemote(name) {
    const remote = new PunchRemoteDB({
      name,
      swarm: this._swarm,
      store: this._store,
      blind: this._blind
    })

    await remote.ready()

    await this._db.insert('@punch-local/repos', {
      name: name,
      key: remote.key
    })

    await this._db.flush()

    this._remotes.set(name, remote)

    return remote
  }

  async openRemote(opts = {}) {
    const repo = await this._db.get('@punch-local/repos', opts)
    if (repo) {
      const remote = new PunchRemoteDB({
        ...repo,
        swarm: this._swarm,
        store: this._store,
        blind: this._blind
      })

      await remote.ready()

      return remote
    }

    return this.joinRemote(opts)
  }

  async joinRemote(opts = {}) {
    const remote = new PunchRemoteDB({
      ...opts,
      swarm: this._swarm,
      store: this._store,
      blind: this._blind
    })

    // Let the remote try getting the rest of the data
    await remote.ready()

    await remote.waitForPeers()

    // Request peering for remote

    await this._db.insert('@punch-local/repos', {
      name: opts.name,
      key: remote.key
    })

    await this._db.flush()

    this._remotes.set(opts.name, remote)

    return remote
  }

  get swarm() {
    return this._swarm
  }

  async getRepo(name) {
    const repo = await this._db.get('@punch-local/repos', { name })
    if (!repo) {
      return null
    }

    const remote = new PunchRemoteDB({
      ...repo,
      swarm: this._swarm,
      store: this._store,
      blind: this._blind
    })

    await remote.ready()

    return remote
  }

  async _open() {
    this._swarm = new Hyperswarm()

    this._wakeup = new Wakeup()
    this._blind = new BlindPeering(this._swarm, this._store, {
      wakeup: this._wakeup,
      mirrors: blindPeers
    })
    this._db = HyperDB.bee(this._store.get({ name: 'db' }), def)

    this._swarm.on('connection', (conn) => {
      this._store.replicate(conn)
      this._wakeup.addStream(conn)

      this.emit('connection', conn)
    })

    await this._db.ready()
  }

  async _close() {
    if (this.blind) await this._blind.close()
    if (this.swarm) await this._swarm.destroy()
    if (this.db) await this._db.close()
    for (const remote of this._remotes.values()) {
      await remote.close()
    }
    await this._store.close()
  }
}

module.exports = {
  PunchLocalDB
}
