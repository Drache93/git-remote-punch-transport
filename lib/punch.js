...
class PunchGit extends ReadyResource {
  ...
  async _open () {
    ...
    let remote = await this._local.getRepo(this._remote)

    if (!remote) {
      remote = await this._local.addRemote(this._remote, this._key, this._discoveryKey)
    }
  ...
  async listForPush () {
    this._verbose('Listing for push to ' + this._remote)
    ...
    for (const ref of this._pendingFetches) {
      ...
      for await (const objectDetails of objectStream) {
        ...
        try {
          ...
          this._debug(`Pushed Object: ${sha} ${type} ${size}`)
        } catch (e) {
      ...
      const unpackResult = await new Promise((resolve, reject) => {
        unpack.once('close', (code) => {
          ...
          this._debug(`Unpacked exit: ${code}`)
          resolve(code)
        })
      })
