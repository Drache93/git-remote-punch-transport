...
class PunchGit extends ReadyResource {
  ...
  async fetch () {
    ...
    for (const ref of this._pendingFetches) {
      ...
      const unpack = await sh.pipe(`cat ${packFiles}`, { cwd: dirname(process.env.GIT_DIR) })
        .pipe('git unpack-objects')
        .exec()
  ...
  async _getRefData (ref) {
    ...
    try {
      ...
      const objectsResult = await sh.pipe(`git rev-list --objects ${ref.value}`)
        .pipe('cut -d\' \' -f1')
