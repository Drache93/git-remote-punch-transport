...
class PunchGit extends ReadyResource {
  ...
  async _open () {
    ...
    const discovery = this._swarm.join(this._topic)
  ...
  async fetch () {
    ...
    for (const ref of this._pendingFetches) {
      ...
      for (const sha of value) {
        ...
        this._debug(`Blob: ${JSON.stringify(objectDetails.id)}`)
        try {
          ...
          this._debug(`Pushed Object: ${sha} ${objectDetails.type} ${objectDetails.size}`)
        } catch (e) {
