...
class PunchGit extends ReadyResource {
  ...
  async _open() {
    ...
    let remote = await this._local.getRepo(this._name)

    if (!remote) {
      remote = await this._local.joinRemote({ key: this._key, name: this._name })
    }
  ...
  async listForPush() {
    ...
    for (const ref of refsList) {
      ...
      if (name.startsWith('refs/heads/')) {
        ...
        const existingRef = await this._remoteDb.get(ref.oid)
  ...
  async fetch() {
    ...
    for (const ref of this._pendingFetches) {
      ...
      this._debug(`Rebuilding repo: ${this._name}`)
      await rebuildRepo({
        remote: this._remoteName,
  ...
  async push() {
    ...
    for (const ref of this._pendingPushes) {
      ...
      try {
        ...
        const tx = this._remoteDb.pushObjects()

        for (const [key, value] of data) {
          const object = await this._remoteDb.get(key)
          ...
          this._verbose(`Saved: ${key}`)
        }

        await tx.flush()

        await this._remoteDb.putRef(pushedRef, [...data.keys()])
        ...
        if (this._progress) {
          this._progressReporter.reportError(`Failed to push ${pushedRef.value}: ${e.message}`)
        }
