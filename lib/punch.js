  ...
  async listForPush() {
    ...
    for (const ref of refsList) {
      ...
      if (name.startsWith('refs/heads/')) {
        ...
        const r = parseRefValue(branchName === 'HEAD' ? `${oid} HEAD` : branch)

        // Check if we already have it
        const existingRef = await this._remoteDb.getObject(r.oid)

        this._output(formatRef(r))

        if (existingRef) {
          this._debug(`Ref already exists, skipping: ${formatRef(r)}`)
          continue
        }
