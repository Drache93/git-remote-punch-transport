...
class PunchGit extends ReadyResource {
  ...
  async listForPush () {
    ...
    refs.split('\n').forEach(ref => {
      ...
      // TODO: confirm what we wan to target here
      // This was targetting refs/remote but this results in duplicates between local and remote
      if (name.startsWith('refs/heads/main')) {
  ...
  async fetch () {
    this._debug(`Fetch: ${this._pendingFetches}`)
  }
